...
// ── Status config ──────────────────────────────────────────────
const STATUSES = {
  not_started: { label: 'Not Started', icon: Circle,        color: '#94A3B8', bg: 'rgba(255, 255, 255, 0.06)', border: 'rgba(255, 255, 255, 0.15)' },
  in_progress: { label: 'In Progress', icon: Activity,      color: '#38BDF8', bg: 'rgba(56, 189, 248, 0.12)', border: 'rgba(56, 189, 248, 0.3)' },
  done:        { label: 'Done',        icon: CheckCircle2,  color: '#34D399', bg: 'rgba(52, 211, 153, 0.12)', border: 'rgba(52, 211, 153, 0.3)' },
};
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      {open && (
        <div style={{
          ...
          background: '#0F111A', border: '1px solid rgba(255, 255, 255, 0.2)',
          borderRadius: '6px', padding: '6px', minWidth: '150px',
          ...
          {Object.entries(STATUSES).map(([key, s]) => {
            ...
            return (
              <button key={key} onClick={() => { onChange(key); setOpen(false); }} style={{
                ...
                padding: '8px 10px', borderRadius: '4px', border: 'none',
                background: key === value ? s.bg : 'transparent',
                color: key === value ? s.color : '#F8FAFC',
...
function TicketCard({ ticket, status, onStatusChange, onAssignChange, onDelete, users, isAuditor }) {
  const prio = getPriority(ticket?.priority || 'LOW');
  const pc = PRIORITY_CONFIG[prio] || PRIORITY_CONFIG.LOW;
  const isDone = status === 'done';
  const reg = ticket?.regulator === 'SEBI';

  const safeChangeList = Array.isArray(ticket?.change_list) 
    ? ticket.change_list 
    : typeof ticket?.change_list === 'string' 
      ? [ticket.change_list] 
      : [];
    ...
    <div className={`glass-card ${pc.className}`} style={{
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: '13px', color: '#A855F7' }}>
                {ticket?.ticket_id || 'TICK-UNSET'}
              </span>
              <span className={`badge ${reg ? 'badge-sebi' : 'badge-rbi'}`}>
                {ticket?.regulator || 'SEBI'}
              </span>
              <span className={`badge ${prio === 'HIGH' ? 'badge-p1' : prio === 'MEDIUM' ? 'badge-p2' : 'badge-p3'}`}>
                {prio}
              </span>
            </div>
            <div style={{ fontSize: '13px', color: '#94A3B8', marginTop: 3 }}>
              Domain: <strong style={{ color: '#F8FAFC' }}>{ticket?.domain || 'General'}</strong>
            </div>
          </div>
        </div>
      ...
      <div style={{
        ...
        <div style={{ background: 'rgba(255, 255, 255, 0.04)', border: '1px solid var(--color-border)', borderRadius: 6, padding: 10 }}>
          <div style={{ fontSize: '11px', fontWeight: 700, color: reg ? '#38BDF8' : '#34D399', textTransform: 'uppercase', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 4 }}>
            <Scale size={12} /> Fetched {ticket?.regulator || 'SEBI'} Regulation
          </div>
          <div style={{ fontSize: '13px', fontWeight: 600, color: '#F8FAFC', lineHeight: 1.4 }}>
            {ticket?.circular_title || `${ticket?.regulator || 'SEBI'} Directive #${ticket?.circular_id || ''}`}
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(255, 255, 255, 0.04)', border: '1px solid var(--color-border)', borderRadius: 6, padding: 10 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {ticket?.affected_policies?.length > 0 ? (
              ...
              <span style={{ fontSize: '13px', color: '#94A3B8' }}>BOI Internal Policy Standard</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div style={{ background: 'rgba(255, 255, 255, 0.04)', border: '1px solid var(--color-border)', borderRadius: 8, padding: '12px 14px', marginBottom: 14, flex: 1 }}>
        ...
        <div style={{ fontSize: '14px', color: '#F8FAFC', lineHeight: 1.6, fontWeight: 400 }}>
          {ticket?.summary || 'Regulatory evaluation completed.'}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, padding: '8px 12px', background: 'rgba(255, 255, 255, 0.03)', borderRadius: 6, fontSize: '13px' }}>
        ...
        {isAuditor ? (
          ...
          <select
            ...
            style={{ background: '#0F111A', border: '1px solid rgba(255, 255, 255, 0.15)', borderRadius: 6, padding: '4px 8px', fontSize: '12px', fontWeight: 600, color: '#F8FAFC', outline: 'none' }}
          >
      ...
      {/* ── Action Items & Action Plan Section ── */}
      {safeChangeList.length > 0 && (
        <div style={{ marginBottom: 12, background: 'rgba(255, 255, 255, 0.03)', padding: 10, borderRadius: 6, border: '1px solid rgba(255, 255, 255, 0.1)' }}>
          <div style={{ fontSize: '11px', color: '#94A3B8', fontWeight: 700, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
            Mandatory Compliance Action Plan:
          </div>
          {safeChangeList.slice(0, 3).map((item, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 4 }}>
              <ArrowRight size={13} color="#34D399" style={{ marginTop: 3, flexShrink: 0 }} />
              <span style={{ fontSize: '13px', color: '#F8FAFC', lineHeight: 1.4 }}>{typeof item === 'string' ? item : JSON.stringify(item)}</span>
            </div>
          ))}
        </div>
      )}
...
export default function TicketsView({ tickets = [], loading, currentUser }) {
  ...
  // Sync statuses with incoming DB tickets so all logins see central database updates
  useEffect(() => {
    if (tickets && tickets.length > 0) {
      const initialMap = {};
      tickets.forEach(t => {
        if (t.ticket_id) {
          initialMap[t.ticket_id] = t.status || 'not_started';
        }
      });
      setStatuses(prev => ({ ...initialMap, ...prev }));
    }
  }, [tickets]);
  ...
  const handleArchive = (ticket) => {
    if (isAuditor) return;
    setHistory(prev => [ticket, ...prev.filter(h => h.ticket_id !== ticket.ticket_id)]);
    setStatuses(prev => ({ ...prev, [ticket.ticket_id]: 'done' }));
  };
  ...
  const rawActive = (tickets || []).filter(t => t && !archivedIds.has(t.ticket_id));

  // Sort by Priority Rank (HIGH > MEDIUM > LOW > Archive) and interleave RBI & SEBI
  const prioWeight = (p = '') => {
  ...
  const activeTickets = [...rawActive].sort((a, b) => {
    const diff = prioWeight(b.priority) - prioWeight(a.priority);
    if (diff !== 0) return diff;
    if (a.regulator !== b.regulator) return a.regulator === 'RBI' ? -1 : 1;
  ...
  const visibleTickets = filteredTickets.slice(startIndex, startIndex + CARDS_PER_PAGE);
  ...
  return (
    <div>
      ...
      {/* ── Filter Bar (High-Contrast Styling) ── */}
      <div className="glass-card" style={{ padding: '14px 18px', marginBottom: 20, display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center', background: 'rgba(15, 23, 42, 0.85)', border: '1px solid rgba(255, 255, 255, 0.12)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: '13px', color: '#94A3B8', fontWeight: 600 }}>Regulator:</span>
          <select
            value={regFilter}
            onChange={e => { setRegFilter(e.target.value); setCurrentPage(0); }}
            style={{
              background: '#0F111A',
              color: '#F8FAFC',
          ...
          >
            <option value="ALL" style={{ background: '#0F111A', color: '#F8FAFC' }}>All Regulators</option>
            <option value="SEBI" style={{ background: '#0F111A', color: '#F8FAFC' }}>SEBI</option>
            <option value="RBI" style={{ background: '#0F111A', color: '#F8FAFC' }}>RBI</option>
          </select>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: '13px', color: '#94A3B8', fontWeight: 600 }}>Priority:</span>
          <select
            value={prioFilter}
            onChange={e => { setPrioFilter(e.target.value); setCurrentPage(0); }}
            style={{
              background: '#0F111A',
              color: '#F8FAFC',
              border: '1px solid rgba(255, 255, 255, 0.25)',
          ...
          >
            <option value="ALL" style={{ background: '#0F111A', color: '#F8FAFC' }}>All Priorities</option>
            <option value="HIGH" style={{ background: '#0F111A', color: '#F8FAFC' }}>HIGH (P1)</option>
            <option value="MEDIUM" style={{ background: '#0F111A', color: '#F8FAFC' }}>MEDIUM (P2)</option>
            <option value="LOW" style={{ background: '#0F111A', color: '#F8FAFC' }}>LOW (P3)</option>
          </select>
        </div>
      </div>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setCurrentPage(p => Math.max(0, p - 1))} disabled={!canPrev} className="action-btn-secondary" style={{ padding: '6px 14px', fontSize: '13px' }}>
                <ChevronLeft size={14} /> Previous
              </button>
              <button onClick={() => setCurrentPage(p => Math.min(totalPages - 1, p + 1))} disabled={!canNext} className="action-btn-secondary" style={{ padding: '6px 14px', fontSize: '13px' }}>
                Next <ChevronRight size={14} />
              </button>
            </div>
            <div style={{ fontSize: '13px', color: 'var(--color-text-muted)', fontWeight: 500, fontFamily: 'var(--font-mono)' }}>
              Page {safePage + 1} of {totalPages} ({filteredTickets.length} total tickets)
            </div>
          </div>
