  ...
  const handleFileChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.name.toLowerCase().endsWith('.pdf') || file.name.toLowerCase().endsWith('.txt')) {
        ...
        alert('Please upload a valid PDF (.pdf) or TXT (.txt) policy document.');
      ...
      <div className="glass-card no-print" style={{ marginBottom: '24px', padding: '24px' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '12px', alignItems: 'center' }}>
            ...
            {/* Scope Badge: Always Compares Against ALL Circulars */}
            <div style={{
              padding: '10px 14px',
              borderRadius: '10px',
              ...
              <span>📜 Audit Scope: Compares against <strong>ALL Master Directions & Circulars</strong> ({availableCirculars.length} total)</span>
              <span style={{ fontSize: '11px', background: 'rgba(52, 211, 153, 0.15)', color: '#34D399', padding: '2px 8px', borderRadius: '12px', fontWeight: 700 }}>Autonomous Audit</span>
            </div>
          </div>
        </div>
